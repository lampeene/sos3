...
function NewSessionContent() {
  ...
  useEffect(() => {
    ...
    placesApi.findAll({}, token).then((res) => {
      setPlaces(res.data || []);
      if (res.data?.length) {
        setForm((p) => ({ ...p, placeId: res.data[0].id }));
      }
    }).catch(() => {});

    // Role Psy = 3, Trainer = 4 (from seed)
    usersApi.findAll({ roleId: 3 }, token).then((res) => {
      setPsys(res.data || []);
    }).catch(() => {});

    usersApi.findAll({ roleId: 4 }, token).then((res) => {
  ...
  return (
    <AdminLayout title="Nouveau stage">
